...
volatile Vector3 volatile transformPoint(Matrix4 matrix, volatile Vector3 point) {
...
result_z              += matrix.m[3][2]*1.0k;
 //printfloat(result_z );
   /*
    printstr("matrix row z:");
    printfloat( matrix.m[0][2]); 
    printfloat(point.x );
    printfloat(matrix.m[1][2]); 
    printfloat(point.y );
    printfloat(matrix.m[2][2]); 
    printfloat(point.z);
    printfloat(matrix.m[3][2]);
    printfloat(1.0k);


    printstr("result via float");
    printfloat(result_x);
   printfloat(result_y);printfloat(result_z);
   */
...
/*
  ...
  result.z*=w;
*/
volatile int a=from_sa(result_x), b=from_sa(result_y),c=from_sa(result_z);
/*
  printvectori("result:",a,b,c);
printfloat(result_z);
  */  
...
/*
    ...
    short accum divider= 1.0;;//1.0k/ result.w;
...
//    unsigned char is_valid=1;
  ...
  // = { x,y,z}; // Expected result: (0.00, 0.00, 0.00)
  Vector3 transformedPoint;
...
//  printvectorf("blip",transformedPoint.x,transformedPoint.y,transformedPoint.z);
